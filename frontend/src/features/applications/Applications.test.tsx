...
describe('applications list', () => {
  ...
  it('groups regional releases by application name and shows their platform IDs', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Show deployment targets for nginx' }))
  ...
  it('applies bookmarked filters from the URL without filtering the fetch', async () => {
    ...
    expect(requests.every((url) => !url.includes('search=') && !url.includes('status='))).toBe(true)
  })
  ...
  it('pages through results at the chosen page size', async () => {
    ...
    await user.selectOptions(screen.getByRole('combobox', { name: 'Applications per page' }), '25')
...
describe('application detail', () => {
  it('renders compact deployment identities with namespaces and Argo deep links', async () => {
    ...
    expect(within(failing).getByRole('link', { name: 'Open prod-eu in Argo CD' })).toHaveAttribute(
      'href',
    ...
    // The breadcrumb names the application rather than showing its ID.
    expect(
      within(screen.getByRole('navigation', { name: 'Breadcrumb' })).getByText('payments-api'),
  ...
  it('lists Kubernetes resources in ownership order and shows their detail', async () => {
    ...
    const graph = await screen.findByRole('list', { name: 'Resources on prod-us-east' })
    ...
    await user.click(screen.getByRole('button', { name: 'Actions for ReplicaSet payments-api-1' }))
  ...
  it('shows declarative manifests without Pods or ReplicaSets', async () => {
    ...
    expect(
      yamlButton.compareDocumentPosition(screen.getByRole('button', { name: 'Deploy' })) &
    ...
    expect(within(modal).getByRole('tablist', { name: 'Resource manifests' })).toBeInTheDocument()
    ...
    const deploymentDiff = await within(modal).findByLabelText('Manifest diff for payments-api')
    expect(deploymentDiff).toHaveTextContent('Helm rendered')
    // Deployed-to header, cluster switch, and the diff's live column.
    ...
    expect(within(modal).getByRole('button', { name: 'Generated fields hidden' })).toHaveAttribute(
      'aria-pressed',
      'true',
    )

    await user.click(within(modal).getByRole('button', { name: 'Generated fields hidden' }))
    expect(await within(modal).findByText('uid: generated')).toBeInTheDocument()

    await user.click(within(modal).getByRole('tab', { name: /Service payments-service/ }))
    expect(
  ...
  it('deletes a resource only after the warning is confirmed', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Delete Deployment payments-api' }))
  ...
  it('shows only Info and Delete when a resource card is clicked', async () => {
    ...
    expect(screen.getByRole('alertdialog')).toHaveTextContent('Delete Deployment payments-api?')
  })
  ...
  it('lists every regional release for the selected application', async () => {
    ...
    expect(screen.getByRole('button', { name: 'View dev-us-east-2 release' })).toBeInTheDocument()
