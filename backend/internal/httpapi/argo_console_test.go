...
func TestRevisionRoutes(t *testing.T) {
	for _, test := range []struct {
		...
		// The service reports a sha outside the values file's history as not found.
		{name: "missing revision", path: "/revisions/abc1234/values", err: onboarding.ErrRevisionNotFound,
			wantStatus: http.StatusNotFound, wantBody: "values file not found at that revision"},
